...
def all_complete():
    """
    역량 점수 6개를 모두 선택하고,
    한 줄 회고는 1개 이상 작성하면 최종 대시보드로 이동할 수 있습니다.
    """
    return score_count() == 6 and reflection_count() >= 1
...
if st.session_state.page == "score":
    ...
    # 역량 점수 6개를 모두 선택하면 버튼이 즉시 활성화되고
    # 회색에서 파란색(primary) 버튼으로 변경됩니다.
    score_completed = score_count() == 6
    ...
    with nav_col:
        if st.button(
            "회고 작성으로 이동  →",
            use_container_width=True,
            type="primary" if score_completed else "secondary",
            disabled=not score_completed,
            key="goto_reflection",
        ):
            go_to("reflection")
...
# =========================================================
    ...
    st.markdown(
        ...
        <div class="section-desc">
            ...
            한 항목 이상 작성하면 최종 대시보드를 확인할 수 있습니다.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with final_col:
        reflection_ready = all_complete()

        if st.button(
            "최종 대시보드 및 회고 카드 확인하기  →",
            use_container_width=True,
            type="primary" if reflection_ready else "secondary",
            disabled=not reflection_ready,
            key="goto_report",
        ):
            go_to("report")
